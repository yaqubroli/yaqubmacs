...
#+end_src
*** Org-ref
Better citation management than how org does it by default.

This is, near-verbatim, the setup code on the official [[https://github.com/jkitchin/org-ref/blob/master/org-ref.org][org-ref manual]].

The only difference is I removed some stuff that seems to only work if you're a developer (i.e. have ~org-ref-ivy~ in your load-path and load it seperately, instead of the MELPA way where it needs to be ~required~ within org-ref's main ~use-package~ declaration.)
...
#+begin_src emacs-lisp
  (use-package ivy-bibtex
    ...
    (setq bibtex-completion-bibliography '("~/Books/References.bib"
          ...
          bibtex-completion-library-path '("~/Books"
                                           "~/Articles")
          bibtex-completion-notes-path "~/Orgfiles/roam/"
          ...
          (lambda (fpath) find-file-other-window fpath)))
  ...
  (use-package org-ref
    ...
    ;; Require the org-ref-ivy and other org-ref related packages within org-ref's config
    (require 'org-ref-ivy)
